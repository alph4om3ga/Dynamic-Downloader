...
**How to apply:** First perform and verify a protected workflow-file update on the target branch. If the OAuth connector remains scoped to `repo` after reauthorization, or the configured Git remote cannot authenticate a push, obtain a repository credential that explicitly supports workflow writes before publishing tags. Do not treat successful unreferenced Git blob uploads as proof of workflow-write access; later tree or commit operations may still be denied. Then confirm the tagged hosted workflow, exactly one release asset, and a Windows x64 launch.